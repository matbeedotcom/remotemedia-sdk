...
/// Dockerized node configuration (combines manifest config with node ID and type)
#[derive(Debug, Clone)]
...
impl DockerizedNodeConfiguration {
    /// Create from node ID, type, and manifest config
    pub fn new(node_id: String, node_type: String, config: DockerExecutorConfig) -> Self {
    ...
    /// Create from node ID only (backward compat - used in tests)
    pub fn new_without_type(node_id: String, config: DockerExecutorConfig) -> Self {
        Self::new(node_id, "Unknown".to_string(), config)
    }
...
pub struct ResourceLimits {
    ...
    /// GPU device IDs to expose (e.g., ["0", "1"] or ["all"])
    #[serde(default)]
...
impl ResourceLimits {
    ...
    pub fn to_docker_host_config(&self) -> bollard::models::HostConfig {
        ...
        if !self.gpu_devices.is_empty() {
            let device_ids = if self.gpu_devices.len() == 1 && self.gpu_devices[0] == "all" {
                vec![]  // Empty means all GPUs
            } else {
                self.gpu_devices.clone()
            };

            host_config.device_requests = Some(vec![DeviceRequest {
                driver: Some("nvidia".to_string()),
                count: if device_ids.is_empty() { Some(-1) } else { None },
                device_ids: if device_ids.is_empty() { None } else { Some(device_ids) },
                capabilities: Some(vec![vec!["gpu".to_string(), "utility".to_string(), "compute".to_string()]]),
                options: None,
            }]);
        }
