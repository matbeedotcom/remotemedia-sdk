...
use remotemedia_runtime::grpc_service::generated::{
    ...
    AudioFormat, DataBuffer, ExecuteRequest, JsonData,
};
...
async fn test_memory_per_execution() {
    ...
    let audio_buffer =
        crate::grpc_integration::test_helpers::create_test_audio_buffer(16000, 1, 440.0);
    ...
    for i in 0..num_concurrent {
        ...
        let handle = tokio::spawn(async move {
            ...
            let manifest = crate::grpc_integration::test_helpers::create_passthrough_manifest(
                &format!("mem_test_{}", i),
            );
...
async fn test_memory_leak_detection() {
    ...
    if memory_samples.len() >= 2 {
        ...
        println!(
            "  First sample: {:.2} MB",
            first_sample as f64 / 1_000_000.0
        );
        println!("  Last sample: {:.2} MB", last_sample as f64 / 1_000_000.0);
        println!(
            "  Growth: {:.2} MB ({:.1}%)",
            memory_growth as f64 / 1_000_000.0,
            growth_percent
        );
...
async fn test_large_audio_buffer_memory() {
    ...
    let large_audio =
        crate::grpc_integration::test_helpers::create_test_audio_buffer(48000, 10, 440.0);
    ...
    assert_eq!(
        successful, 10,
        "Expected all large audio requests to succeed"
    );
