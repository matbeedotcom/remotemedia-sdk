...
async fn test_connection_reuse() {
    ...
    let manifest =
        crate::grpc_integration::test_helpers::create_calculator_manifest("reuse_test", "add", 1.0);
    ...
    for i in 0..10 {
        ...
        data_inputs.insert(
            ...
            DataBuffer {
                data_type: Some(data_buffer::DataType::Json(JsonData {
                    json_payload: format!(r#"{{"value": {}.0}}"#, i),
        ...
        assert!(
            matches!(inner.outcome, Some(Outcome::Result(_))),
...
async fn test_connection_timeout_handling() {
    ...
    assert!(
        result.is_ok(),
        "Connection with reasonable timeout should succeed"
    );
    assert!(result.unwrap().is_ok(), "Connection should be successful");
