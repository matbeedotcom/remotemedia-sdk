...
mod tests {
    use remotemedia_runtime::python::multiprocess::{
        MultiprocessExecutor, MultiprocessConfig, InitStatus,
    };
    ...
    async fn test_detect_process_crash() {
        ...
        assert!(progress.is_ok(), "Should be able to get progress for active session");
    ...
    async fn test_cascade_termination_on_crash() {
        ...
        assert!(progress_after.is_err(), "Session should no longer exist after termination");
    }
    ...
    async fn test_concurrent_session_isolation() {
        ...
        executor.terminate_session("session_b")
            .await
        ...
        for session_id in &["session_a", "session_c"] {
            let progress = executor.get_init_progress(session_id).await;
            assert!(progress.is_ok(), "Session {} should still exist", session_id);
        }
    ...
    async fn test_process_limit_enforcement() {
        ...
        for i in 0..3 {
            ...
            executor.update_init_progress(
                ...
                1.0,
                "Ready".to_string(),
            ).await.expect("Failed to add node within limit");
        }
    ...
    async fn test_config_loading_from_toml() {
        ...
        let config = MultiprocessConfig::from_toml_str(toml_str)
        ...
        assert_eq!(config.python_executable.to_str().unwrap(), "/usr/bin/python3");
        assert!(!config.enable_backpressure);
    }
