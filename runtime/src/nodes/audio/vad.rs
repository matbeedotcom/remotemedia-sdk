...
impl RustVADNode {
    ...
    fn compute_frame_energy(&mut self, frame: &[f32]) -> f32 {
        ...
        let windowed: Vec<Complex<f32>> = frame.iter().enumerate().map(|(i, &sample)| {
            let window = 0.54 - 0.46 * (2.0 * std::f32::consts::PI * i as f32 / (frame_size - 1) as f32).cos();
    ...
    async fn process(&mut self, input: Value) -> Result<Vec<Value>> {
        ...
        if sample_rate != self.sample_rate {
            return Err(Error::Execution(
                format!("Input sample rate {} doesn't match expected {}", sample_rate, self.sample_rate)
            ));
        }
...
impl VADNodeFactory {
    pub fn new(sample_rate: u32, frame_duration_ms: u32, energy_threshold: f32) -> Self {
        Self { sample_rate, frame_duration_ms, energy_threshold }
    }
}
