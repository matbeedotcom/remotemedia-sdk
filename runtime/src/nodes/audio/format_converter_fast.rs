    ...
    fn process_audio(&mut self, input: AudioData) -> Result<AudioData> {
        ...
        let converted_buffer = match (source_format, self.target_format) {
            (AudioFormat::F32, AudioFormat::I16) => {
                let data = input.buffer.as_f32().ok_or_else(|| {
                    Error::Execution("Expected F32 buffer".to_string())
                })?;
            ...
            (AudioFormat::I16, AudioFormat::F32) => {
                let data = input.buffer.as_i16().ok_or_else(|| {
                    Error::Execution("Expected I16 buffer".to_string())
                })?;
            ...
            (AudioFormat::I32, AudioFormat::F32) => {
                let data = input.buffer.as_i32().ok_or_else(|| {
                    Error::Execution("Expected I32 buffer".to_string())
                })?;
...
mod tests {
    ...
    fn test_f32_to_i16_conversion() {
        ...
        let input = AudioData::new(
            AudioBuffer::new_f32(input_data),
            44100,
            1,
        );
    ...
    fn test_no_conversion_needed() {
        ...
        let input = AudioData::new(
            AudioBuffer::new_f32(input_data.clone()),
            44100,
            1,
        );
