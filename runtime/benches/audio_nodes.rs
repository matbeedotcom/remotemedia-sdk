...
fn create_test_audio_i32(num_channels: usize, num_samples: usize) -> Vec<i32> {
    let f32_audio = create_test_audio_f32(num_channels, num_samples);
    f32_audio.iter().map(|&x| (x * 2147483647.0) as i32).collect()
}
            ...
            group.bench_with_input(
                ...
                |b, audio| {
                    ...
                    b.to_async(&rt).iter(move || {
                        ...
                        async move {
                            let mut node = RustResampleNode::new(
                                input_sr,
                                output_sr,
                                q.clone(),
                                num_channels,
        ...
        |b, audio| {
            b.to_async(&rt).iter(move || {
                ...
                async move {
                    ...
                    let resample_output = resample_node.process(vad_output[0].clone()).await.unwrap();
                    
                    // Stage 3: Format conversion
                    let final_output = format_node.process(resample_output[0].clone()).await.unwrap();
