// Build script for remotemedia-ingest-srt
...
fn main() {
    ...
    // OUT_DIR is like: target/debug/build/ingest-srt-xxx/out
    // We need:         target/debug/build/ac-ffmpeg-xxx/out
    let build_dir = out_path
        .parent() // ingest-srt-xxx/out -> ingest-srt-xxx
        .and_then(|p| p.parent()) // ingest-srt-xxx -> build
