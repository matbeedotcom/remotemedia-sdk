//! Proc-macros for RemoteMedia core
//!
...
fn generate_schema_impl(
    ...
    quote! {
        impl remotemedia_core::nodes::schema::NodeConfigSchema for #config_name {
            ...
            fn accepts() -> Vec<remotemedia_core::nodes::schema::RuntimeDataType> {
                vec![#accepts_tokens]
            }

            fn produces() -> Vec<remotemedia_core::nodes::schema::RuntimeDataType> {
                vec![#produces_tokens]
            }
...
fn generate_trait_impl(
    ...
    let process_impl = if multi_output {
        quote! {
            async fn process(&self, _data: remotemedia_core::data::RuntimeData) -> Result<remotemedia_core::data::RuntimeData, remotemedia_core::Error> {
        ...
        quote! {
            async fn process(&self, data: remotemedia_core::data::RuntimeData) -> Result<remotemedia_core::data::RuntimeData, remotemedia_core::Error> {
    ...
    let streaming_impl = if multi_output {
        quote! {
            async fn process_streaming<F>(
                &self,
                data: remotemedia_core::data::RuntimeData,
                session_id: Option<String>,
                callback: F,
            ) -> Result<usize, remotemedia_core::Error>
            where
                F: FnMut(remotemedia_core::data::RuntimeData) -> Result<(), remotemedia_core::Error> + Send,
            {
    ...
    quote! {
        ...
        impl remotemedia_core::nodes::AsyncStreamingNode for #struct_name {
...
fn generate_inventory_registration(struct_name: &syn::Ident) -> proc_macro2::TokenStream {
    ...
    quote! {
        inventory::submit! {
            remotemedia_core::nodes::schema::RegisteredNodeConfig::new(
                <#config_name as remotemedia_core::nodes::schema::NodeConfigSchema>::to_node_schema
            )
        }
    }
}
...
fn parse_capability_behavior(behavior: &str) -> proc_macro2::TokenStream {
    match behavior.to_lowercase().as_str() {
        "static" => quote! { remotemedia_core::capabilities::CapabilityBehavior::Static },
        "configured" => quote! { remotemedia_core::capabilities::CapabilityBehavior::Configured },
        "passthrough" => quote! { remotemedia_core::capabilities::CapabilityBehavior::Passthrough },
        "adaptive" => quote! { remotemedia_core::capabilities::CapabilityBehavior::Adaptive },
        "runtime_discovered" | "runtimediscovered" => {
            quote! { remotemedia_core::capabilities::CapabilityBehavior::RuntimeDiscovered }
        }
...
fn parse_constraint_value_dsl(value: &str, field_type: &str) -> proc_macro2::TokenStream {
    ...
    if trimmed.contains("..") {
        ...
        if parts.len() == 2 {
            ...
            if let (Ok(min_val), Ok(max_val)) = (min.parse::<u32>(), max.parse::<u32>()) {
                return quote! {
                    Some(remotemedia_core::capabilities::ConstraintValue::Range {
                        min: #min_val,
                        max: #max_val,
                    })
                };
    ...
    if trimmed.starts_with('[') && trimmed.ends_with(']') {
        ...
        if let Ok(int_vals) = parsed_ints {
            return quote! {
                Some(remotemedia_core::capabilities::ConstraintValue::Set(
                    vec![#(#int_vals),*]
                ))
            };
        }
    ...
    if let Ok(int_val) = trimmed.parse::<u32>() {
        return quote! {
            Some(remotemedia_core::capabilities::ConstraintValue::Exact(#int_val))
        };
    }
    ...
    if let Ok(float_val) = trimmed.parse::<f32>() {
        return quote! {
            Some(remotemedia_core::capabilities::ConstraintValue::Exact(#float_val))
        };
    }
    ...
    if field_type == "format" {
        ...
        return quote! {
            Some(remotemedia_core::capabilities::ConstraintValue::Exact(#format_token))
        };
    }

    // String value
    quote! {
        Some(remotemedia_core::capabilities::ConstraintValue::Exact(#trimmed.to_string()))
    }
}
...
fn parse_audio_format_dsl(format: &str) -> proc_macro2::TokenStream {
    match format.to_uppercase().as_str() {
        "F32" => quote! { remotemedia_core::capabilities::AudioSampleFormat::F32 },
        "I16" => quote! { remotemedia_core::capabilities::AudioSampleFormat::I16 },
        "I32" => quote! { remotemedia_core::capabilities::AudioSampleFormat::I32 },
        "U8" => quote! { remotemedia_core::capabilities::AudioSampleFormat::U8 },
...
fn parse_media_constraints_dsl(dsl: &str) -> proc_macro2::TokenStream {
    ...
    match trimmed.to_lowercase().as_str() {
        "binary" => return quote! { remotemedia_core::capabilities::MediaConstraints::Binary },
        "text" => return quote! { remotemedia_core::capabilities::MediaConstraints::Text(Default::default()) },
        "json" => return quote! { remotemedia_core::capabilities::MediaConstraints::Json(Default::default()) },
        _ => {}
    }
    ...
    let paren_start = match trimmed.find('(') {
        ...
        None => {
            ...
            return match trimmed.to_lowercase().as_str() {
                "audio" => quote! { remotemedia_core::capabilities::MediaConstraints::Audio(Default::default()) },
                "video" => quote! { remotemedia_core::capabilities::MediaConstraints::Video(Default::default()) },
                "tensor" => quote! { remotemedia_core::capabilities::MediaConstraints::Tensor(Default::default()) },
                "file" => quote! { remotemedia_core::capabilities::MediaConstraints::File(Default::default()) },
    ...
    match media_type.as_str() {
        "audio" => {
            ...
            quote! {
                remotemedia_core::capabilities::MediaConstraints::Audio(
                    remotemedia_core::capabilities::AudioConstraints {
        ...
        "video" => {
            ...
            quote! {
                remotemedia_core::capabilities::MediaConstraints::Video(
                    remotemedia_core::capabilities::VideoConstraints {
                        width: #width,
                        height: #height,
                        framerate: #framerate,
        ...
        "text" => {
            ...
            quote! {
                remotemedia_core::capabilities::MediaConstraints::Text(
                    remotemedia_core::capabilities::TextConstraints {
                        encoding: #encoding,
                        format: #text_format,
                    }
                )
...
fn generate_capability_behavior_impl(
    ...
    quote! {
        impl #struct_name {
            ...
            pub fn capability_behavior_static() -> remotemedia_core::capabilities::CapabilityBehavior {
                #behavior
            }
        }
        ...
        impl #config_name {
            /// Returns the capability behavior for this node type.
            pub fn capability_behavior() -> remotemedia_core::capabilities::CapabilityBehavior {
                #behavior
            }
        }
    }
...
fn generate_media_capabilities_impl(
    ...
    quote! {
        impl #struct_name {
            ...
            pub fn media_capabilities_static() -> remotemedia_core::capabilities::MediaCapabilities {
                use std::collections::HashMap;
                let mut inputs: HashMap<String, remotemedia_core::capabilities::MediaConstraints> = HashMap::new();
                let mut outputs: HashMap<String, remotemedia_core::capabilities::MediaConstraints> = HashMap::new();
                #inputs_code
                #outputs_code
                remotemedia_core::capabilities::MediaCapabilities { inputs, outputs }
            }
        }

        impl #config_name {
            /// Returns the media capabilities for this node type.
            pub fn media_capabilities(_params: &serde_json::Value) -> remotemedia_core::capabilities::MediaCapabilities {
            ...
            let types: Vec<_> = s
                ...
                .map(|t| {
                    let variant = match t.to_lowercase().as_str() {
                        "audio" => quote! { remotemedia_core::nodes::schema::RuntimeDataType::Audio },
                        "video" => quote! { remotemedia_core::nodes::schema::RuntimeDataType::Video },
                        "json" => quote! { remotemedia_core::nodes::schema::RuntimeDataType::Json },
                        "text" => quote! { remotemedia_core::nodes::schema::RuntimeDataType::Text },
                        "binary" => quote! { remotemedia_core::nodes::schema::RuntimeDataType::Binary },
                        "tensor" => quote! { remotemedia_core::nodes::schema::RuntimeDataType::Tensor },
                        "numpy" => quote! { remotemedia_core::nodes::schema::RuntimeDataType::Numpy },
                        "control" | "controlmessage" => quote! { remotemedia_core::nodes::schema::RuntimeDataType::ControlMessage },
