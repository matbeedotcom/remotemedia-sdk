...
impl SyntheticDataFactory {
    ...
    pub fn load_wav(path: &Path) -> Result<RuntimeData, String> {
        ...
        if audio_format != 1 && audio_format != 3 {
            return Err(format!("Unsupported WAV format: {audio_format} (expected PCM=1 or float=3)"));
        }
    ...
    pub fn load_wav_chunked(
        path: &Path,
        chunk_size: usize,
    ) -> Result<Vec<RuntimeData>, String> {
            ...
            RuntimeData::Audio {
                ...
                let chunks: Vec<RuntimeData> = samples
                    ...
                    .map(|(i, chunk)| {
                        ...
                        let timestamp_us =
                            (offset_samples as u64 * 1_000_000) / sample_rate as u64;
                        RuntimeData::Audio {
                    ...
                    .collect();
                info!("Chunked WAV into {} chunks of {chunk_size} samples", chunks.len());
                Ok(chunks)
            }
