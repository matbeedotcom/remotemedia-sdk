...
async function testWebRtcPipelineStreaming() {
    console.log('🧪 Testing WebRTC-like Pipeline Streaming from JavaScript\n');
    ...
    try {
        ...
        const audioPipeline = new PipelineBuilder('JavaScriptWebRtcPipeline')
            .addNode({
                nodeId: 'audio_buffer',
                nodeType: 'AudioBuffer',
                config: { buffer_size_samples: 8192 }
            })
            .addNode({
                nodeId: 'vad',
            ...
            .setMetadata({
                ...
                description: 'Pipeline mimicking webrtc_pipeline_server.py for JS testing',
                created_by: 'javascript_client'
            })
            .build();

        pipelineId = await client.registerPipeline(
            'js_webrtc_pipeline_test',
            audioPipeline,
            {
                metadata: {
        ...
        const transcriptionPromise = new Promise((resolve, reject) => {
            ...
            stream.on('ready', (sessionId) => {
                ...
                reader.on('end', () => {
                    ...
                    setTimeout(() => stream.close(), 1000); // Allow time for pipeline to flush
                });
        ...
        console.log('\n✅ WebRTC-like streaming test completed successfully!');
