...
fn profile_format_conversion(c: &mut Criterion) {
    ...
    group.bench_function("2_node_creation_only", |b| {
        b.iter(|| black_box(RustFormatConverterNode::new(AudioFormat::I16)));
    });
...
fn profile_resample(c: &mut Criterion) {
    ...
    group.bench_function("2_node_creation", |b| {
        b.iter(|| {
            black_box(RustResampleNode::new(
                44100,
                16000,
    ...
    group.bench_function("3_full_operation", |b| {
        b.to_async(&rt).iter(|| {
            ...
            async move {
                let mut node =
                    RustResampleNode::new(44100, 16000, ResampleQuality::High, num_channels);
                let ctx = create_context();
                node.initialize(&ctx).await.unwrap();
...
fn profile_vad(c: &mut Criterion) {
    ...
    // 2. Node creation
    group.bench_function("2_node_creation", |b| {
        b.iter(|| black_box(RustVADNode::new(sample_rate as u32, 30, 0.01)));
    });
...
fn profile_step_by_step(c: &mut Criterion) {
    ...
    let start = Instant::now();
    let result = rt.block_on(async { node.process(input_json).await.unwrap() });
    ...
    println!(
        ...
        (json_time + node_creation_time + init_time + serialize_time).as_micros() as f64
            / (json_time + node_creation_time + init_time + process_time + serialize_time)
                .as_micros() as f64
            * 100.0
    );
